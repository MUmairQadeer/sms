...
global $db_conn;
$output =[];
$query =mysqli_query($db_conn,"SELECT *  FROM usermetadata WHERE `user_id`='$user_id'");
while($result =mysqli_fetch_object($query)){
    $output[] =$result;
}
return $output;
...
function get_usermeta($user_id,$meta_key,$signal=true){
    global $db_conn;
   if(!empty($user_id) && !empty($meta_key)){
    $query =mysqli_query($db_conn,"SELECT *  FROM usermetadata WHERE `user_id`='$user_id' AND 
   `meta_key`='$meta_key'");
   }
   else{
    return false ;
